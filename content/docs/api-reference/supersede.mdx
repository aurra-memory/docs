...
At least one of `superseded_by` or `valid_to` must be provided. Both may be supplied together (e.g. to supersede with a backdated `valid_to`).
...
| Status | Trigger |
| --- | --- |
| `400` | Neither `superseded_by` nor `valid_to` provided. (Supplying both is allowed — they apply together.) |
