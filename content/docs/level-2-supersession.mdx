...
4.  Classify (lightweight Claude Haiku call):
     - "supersedes existing #17" -> supersede #17
     - "refines existing #17" -> system policy: keep both rows for now
     - "independent" -> new row, no supersession
...
| `supersession.verdict` | One of `supersedes`, `refines`, or `independent`. (Dedup of exact-text duplicates happens earlier in the write path; see [duplicate handling](#duplicate-handling) below.) |
| `supersession.confidence` | 0-1 float. Aurra rejects anything below `min_confidence`. |
| `supersession.reasoning` | The classifier's explanation. Show this to users in audit UIs. |

## Duplicate handling

Aurra also has a separate, simpler dedup pass that runs **before** the classifier on every write:

- **Scope:** dedup is per-tenant. Memories from different tenants never collide.
- **Key for Slack messages:** `(company_id, tenant_id, slack_ts)` - exact-match.
- **Key for all other writes:** `(company_id, tenant_id, source, channel)` plus the first 100 characters of the `decision` text.
- **Behavior:** if the dedup key matches an existing row, the write is silently skipped (returns no new memory id; check `extracted_count` vs `saved_count` in the response to detect).

This dedup is **fingerprint-based, not semantic**. Two memories with similar but differently-worded decisions ("user loves pickleball" vs "user enjoys pickleball") will both save. Use Level 2 supersession for semantic-aware merging.

## Cost and latency

Auto-supersession runs on every write that passes the category gate. The classifier itself (one Claude Haiku call) typically takes 1-3 seconds. Including embedding generation, semantic search, and database writes, **the full auto-supersession flow is typically 5-15 seconds end-to-end** — usually 5-10 seconds when no similar memories exist (classifier doesn't run), longer when one or more candidates need classification. Per Haiku call costs under $0.0001. Excluded categories and disabled keys add zero overhead.
